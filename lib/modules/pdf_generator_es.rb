  ...
  def descripcio_pdf_es(sistema, sistema_usuari)
    ...
    sistema_seleccionat.attributes.each_pair do |name, value|
      if (value == true)
        ...
        llistat_elements = llistat_elements + '<li class="li-impressio">' + element.descripcio_es + '</li>'
      end
    end
    ...
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_habitatges != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en viviendas: ' + @edifici.estructura.sobrecarregues_habitatges.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_zones_comuns != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en zonas comunes: ' + @edifici.estructura.sobrecarregues_zones_comuns.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_coberta_inclinada != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en cubierta inclinada: ' + @edifici.estructura.sobrecarregues_coberta_inclinada.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_coberta_plana != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en cubierta plana: ' + @edifici.estructura.sobrecarregues_coberta_plana.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_locals != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en locales: ' + @edifici.estructura.sobrecarregues_locals.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_garatges != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en garajes: ' + @edifici.estructura.sobrecarregues_garatges.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'estructura' && @edifici.estructura.sobrecarregues_trasters != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecargas admisibles en trasteros: ' + @edifici.estructura.sobrecarregues_trasters.to_s + ' kg/m<sup>2</sup></li>'
    end
    ...
    if sistema_usuari == 'tancaments' && @edifici.tancaments_vertical.sobrecarrega_repartida_balco != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecarga repartida del balcón: ' + @edifici.tancaments_vertical.sobrecarrega_repartida_balco.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'tancaments' && @edifici.tancaments_vertical.sobrecarrega_linial_vora_balco != nil
      llistat_elements = llistat_elements + '<li class="li-impressio">Sobrecarga lineal actuando en el borde del balcón: ' + @edifici.tancaments_vertical.sobrecarrega_linial_vora_balco.to_s + ' kg/m</li>'
    end
    #Tanquem el llistat
...
<p class="p-impressio">Hay que evitar cualquier tipo de cambio en el sistema de carga de las diferentes partes del edificio. Si desea introducir modificaciones o cualquier cambio de uso dentro del edificio consulte a su Técnico de Cabecera.</p>
<p class="p-impressio">Las lesiones (grietas, desplomes) en la cimentación no son apreciables directamente y se detectan a partir de las que aparecen en otros elementos constructivos (paredes, techos, etc.). En estos casos, hace falta que el Técnico de Cabecera haga un informe sobre las lesiones detectadas, determine la gravedad y, en su caso, la necesidad de intervención.</p>
<p class="p-impressio">Las alteraciones de importancia efectuadas en los terrenos próximos, tales como: nuevas construcciones, realización de pozos, túneles, vías, carreteras o rellenos de tierras pueden afectar a la cimentación del edificio. Si durante la realización de los trabajos se detectan lesiones, se deberán estudiar y, en su caso, se podrá exigir su reparación.</p>
<p class="p-impressio">Las corrientes subterráneas de agua natural y las fugas de conducciones de agua o de desagües pueden ser causa de alteraciones del terreno y de descalces de la cimentación. Estos descalces pueden producir un asentamiento de la zona afectada, que puede transformarse en deterioros importantes en el resto de la estructura. Por este motivo, es primordial eliminar rápidamente cualquier tipo de humedad que provenga del subsuelo.</p>
<p class="p-impressio">Después de lluvias fuertes se observan las posibles humedades y el buen funcionamiento de las perforaciones de drenaje y desagüe.</p> 
<div class="salt-pagina"></div>'

  	when 'estructura'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Las humedades persistentes en los elementos estructurales tienen un efecto nefasto sobre la conservación de la estructura. Hay que reparar inmediatamente.</p>
<p class="p-impressio">Si se ha de colgar objetos (cuadros, estanterías, muebles o luminarias) a los elementos estructurales a utilizar tacos y tornillos adecuados para el material de base.</p>
<p class="p-impressio">Los elementos que forman parte de la estructura del edificio, paredes de carga incluidas, no se pueden alterar sin el control del Técnico de Cabecera. Esta prescripción incluye la realización de regatas en las paredes de carga y la apertura de pasos para la redistribución de espacios interiores.</p>
<p class="p-impressio">Durante la vida útil del edificio pueden aparecer síntomas de lesiones en la estructura o en los elementos en contacto con ella. En general, estos defectos pueden tener carácter grave. En estos casos, es necesario que su Técnico de Cabecera analice las lesiones detectadas, determine su importancia y, en su caso, decida la necesidad de una intervención.</p>
<p class="p-impressio">Relación orientativa de síntomas de lesiones con posible repercusión sobre la estructura:
<ul>
<li class="li-impressio">Deformaciones: desplomes de paredes, fachadas y pilares.</li>
<li class="li-impressio">Fisuras y grietas: en paredes, fachadas y pilares.</li>
<li class="li-impressio">Manchas de óxido en elementos metálicos.</li>
</ul>
</p>
<div class="salt-pagina"></div>'
  	...
  	when 'tancaments'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Las fachadas separan la vivienda del ambiente exterior, por este motivo, deben cumplir importantes exigencias de aislamiento respecto del frío o el calor, el ruido, la entrada de aire y de humedad, la resistencia, la seguridad el robo, etc.</p>
<p class="p-impressio">La fachada constituye la imagen externa de la casa y de sus ocupantes, conforma la calle y, por tanto, configura el aspecto de nuestra ciudad. Por ello, no puede alterarse (cerrar balcones con vidrio, abrir nuevas aperturas, instalar toldos o rótulos no apropiados) sin tener en cuenta las ordenanzas municipales y la aprobación de la Comunidad de Propietarios.</p> 
<p class="p-impressio">Los acabados de la fachada suelen ser unos de los puntos más frágiles del edificio ya que están en contacto directo con la intemperie. Por otra parte, lo que inicialmente quizás sólo es suciedad o una degradación de la imagen estética de la fachada, se puede convertir en un peligro, ya que cualquier desprendimiento caería directamente sobre la calle.</p>
<p class="p-impressio">Los aplacados de piedra natural se ensucian con mucha facilidad dependiendo de la porosidad de la piedra. Consulte a su Técnico de Cabecera la posibilidad de aplicar un producto protector incoloro.</p>
<p class="p-impressio">Las paredes medianeras son aquellas que separan el edificio de los edificios vecinos. Cuando no haya edificios vecinos o sean más bajos, las medianeras quedarán a la vista y deberán estar protegidas como si fueran fachadas.</p>
<p class="p-impressio">En los balcones y las galerías no se deben colocar cargas pesadas, tales como jardineras o materiales almacenados. También debería evitar que el agua que se utiliza para regar chorrear por la fachada.</p>
<p class="p-impressio">Las ventanas y balcones exteriores son elementos comunes del edificio, aunque su uso sea mayoritariamente privado. Cualquier modificación de su imagen exterior (incluyendo el cambio de perfilería) deberá ser aprobada por la Comunidad de Propietarios. Sin embargo, la limpieza y el mantenimiento corresponde a los usuarios de las viviendas.</p>
<p class="p-impressio">No se apoyarán sobre las ventanas y balcones, elementos de sujeción de andamios, poleas para levantar cargas o muebles, mecanismos de limpieza exteriores u otros objetos que puedan dañar.</p>
<p class="p-impressio">No se deben dar golpes fuertes a las ventanas. Por otra parte, las ventanas pueden conseguir una alta estanqueidad al aire y al ruido con la colocación de burletes especialmente concebidos para este fin.</p>
<p class="p-impressio">Los cristales se deberán limpiar con agua jabonosa, preferentemente tibia, y posteriormente se secarán. No se deben frotar con trapos secos, ya que el vidrio se rayaría.</p>
<p class="p-impressio">En las persianas enrollables de PVC, se debe evitar forzar las lamas cuando pierdan la horizontalidad o se queden atascadas en las guías. Se debe limpiar con detergentes no alcalinos y agua caliente. Es necesario utilizar un paño suave o una esponja.</p>
<p class="p-impressio">Una falta de aislamiento térmico puede ser la causa de la existencia de humedades de condensación. El Técnico de Cabecera deberá analizar los síntomas adecuadamente para determinar posibles defectos en el aislamiento térmico.</p>
<p class="p-impressio">Si el aislamiento térmico se moja, por su efectividad. Por lo tanto, se debe evitar cualquier tipo de humedad que lo pueda afectar.</p>
<p class="p-impressio">El ruido se transmite por el aire o por medio de los materiales del edificio. Puede provenir de la calle o del interior de la casa. El ruido de la calle se puede reducir mediante ventanas con doble acristalamiento o dobles ventanas. El ruido de las personas se puede reducir con la colocación de materiales aislantes o absorbentes acústicos en paredes y techos.</p>
<p class="p-impressio">Para las reposiciones de los elementos que tengan una duración más corta que la propia fachada (juntos, protecciones, etc.) o de los cerramientos de vidrio, se utilizarán productos idénticos a los existentes o de características equivalentes que no alteren sus prestaciones iniciales.</p>
<p class="p-impressio">Los desprendimientos de elementos de la fachada, son un riesgo, tanto para los usuarios como para los peatones.</p>
<p class="p-impressio">Es responsabilidad del usuario, que, cuando haya síntomas de degradaciones, soplados y / o elementos rotos en las fachadas, avisar rápidamente a los responsables de mantenimiento del edificio para que se tomen las medidas oportunas. En caso de peligro inminente hay que avisar al Servicio de Bomberos.</p>
<div class="salt-pagina"></div>'

  	when 'cobertes'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Cubiertas planas:</p>
<p class="p-impressio">Las cubiertas planas deben mantenerse limpias y sin hierbas, especialmente los sumideros, las canales y las limas. Es preferible no colocar jardineras cerca de los desagües o bien que estén elevadas del suelo para permitir el paso del agua.</p>
<p class="p-impressio">Este tipo de cubierta sólo debe utilizarse para el uso a que haya sido proyectada. En este sentido, se evitará el almacenamiento de materiales, muebles, etc., y el vertido de productos químicos agresivos tales como aceites, disolventes o lejía.</p>
<p class="p-impressio">Si en la cubierta se instalan nuevas antenas, equipos de aire acondicionado o, en general, aparatos que requieran ser fijados, la sujeción no debe afectar a la impermeabilización.</p>
<p class="p-impressio">Tampoco se deben utilizar como puntos de anclaje de tensores, de barandillas metálicas o de obra, ni de conductos de evacuación de humos existentes, salvo que el Técnico de Cabecera lo autorice. Si estas nuevas instalaciones precisan de un mantenimiento periódico, se preverán en su entorno las protecciones adecuadas.</p>
<p class="p-impressio">En caso de que se observen humedades en los pisos bajo cubierta, habrá controlarlas, ya que pueden tener un efecto negativo sobre los elementos estructurales.</p>
<p class="p-impressio">Hay que procurar, siempre que sea posible, no caminar por encima de las cubiertas planas no transitables. Cuando sea necesario pisarlas hay que ir con cuidado para no producir desperfectos. El personal de inspección, conservación o reparación estará provisto de zapatos de suela blanda.</p>
<p class="p-impressio">Cubiertas con pendiente:</p>
<p class="p-impressio">Las cubiertas deben mantenerse limpias y sin hierbas, especialmente los sumideros, las canales y las limas. Se procurará, siempre que sea posible, no pisar las cubiertas en pendiente. Cuando se transite hay que tener mucho cuidado para no producir desperfectos.</p>
<p class="p-impressio">Las cubiertas en pendiente serán accesibles sólo para su conservación. El personal encargado del trabajo irá provisto de cinturón de seguridad que sujetará a dos ganchos de servicio o en puntos fijos de la cubierta. Es recomendable que los operarios lleven zapatos con suelas blandas y antideslizantes. No se transitará sobre las cubiertas si están mojadas.</p>
<p class="p-impressio">Si en la cubierta se instalan nuevas antenas, equipos de aire acondicionado o, en general, aparatos que requieran ser fijados, la sujeción no puede afectar a la impermeabilización. Tampoco deben utilizarse como puntos de anclaje de tensores, de barandillas metálicas o de obra, ni de conductos de evacuación de humos existentes, a menos que un técnico especializado lo autorice. Si estas nuevas instalaciones necesitan de un mantenimiento periódico, habrá que prever en su entorno las protecciones adecuadas.</p>
<p class="p-impressio">En caso de que se observen humedades en las plantas bajo cubierta, se deberán controlar, ya que pueden tener un efecto negativo sobre los elementos estructurales.</p>
<p class="p-impressio">El musgo y los hongos se eliminan con un cepillo y si es necesario se aplicará un fungicida.</p>
<p class="p-impressio">Los trabajos de reparación se harán siempre retirando la parte dañada para no sobrecargar la estructura.</p>
<p class="p-impressio">Lucernarios y claraboyas:</p>
<p class="p-impressio">Las claraboyas y tragaluces deben limpiarse con asiduidad, ya que en caso de ensuciarse reducen considerablemente la cantidad de luz que dejan pasar.</p>
<p class="p-impressio">Por su situación dentro del edificio, deben extremarse las medidas de seguridad en el momento de limpiarlas para evitar accidentes.</p>
<div class="salt-pagina"></div>'
  	...
  	when 'sanejament'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">La red de saneamiento se compone básicamente de elementos y conductos de desagüe de los aparatos de cada planta y de algunos recintos del edificio, que conectan con la red de saneamiento vertical (bajantes) y con los sumideros, arquetas, colectores , etc., hasta la red municipal u otro sistema autorizado.</p>
<p class="p-impressio">En la red de saneamiento es muy importante conservar la instalación limpia y libre de depósitos. Se puede conseguir con un mantenimiento reducido basado en una utilización adecuada y en unos hábitos higiénicos correctos por parte de los usuarios.</p>
<p class="p-impressio">La red de evacuación de agua, sobre todo el inodoro, no se puede utilizar como vertedero de desechos. No se pueden tirar plásticos, algodón, gomas, compresas, hojas de afeitar, bastoncillos, etc. Las sustancias y los elementos anteriores, por sí mismos o combinados, pueden taponar o, incluso, destruir por procedimientos físicos o reacciones químicas las conducciones y / o sus elementos, produciéndose derrames malolientes tales como fugas, manchas, etc.</p>
<p class="p-impressio">Se deben revisar con frecuencia los sifones de los sumideros y comprobar que no les falte agua, para evitar que los olores de la red salgan al exterior.</p>
<p class="p-impressio">Para desatascar los conductos no se pueden utilizar ácidos o productos que perjudiquen los desagües. Se utilizarán siempre detergentes biodegradables para evitar la creación de espumas que petrifican dentro de los sifones y las arquetas del edificio. Tampoco se verterán aguas que contengan aceites, colorantes permanentes o sustancias tóxicas. Como ejemplo, un litro de aceite mineral contamina 10.000 litros de agua.</p>
<p class="p-impressio">Cualquier modificación en la instalación o en las condiciones de uso que puedan alterar el normal funcionamiento será realizada mediante un estudio previo y bajo la dirección de su Técnico de Cabecera.</p>
<p class="p-impressio">Las posibles fugas se localizarán y repararán lo antes posible.</p>
<div class="salt-pagina"></div>'

  	when 'aigua'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Responsabilidades:</p>
<p class="p-impressio">El mantenimiento de la instalación a partir del contador (no sólo desde la llave de paso de la vivienda) es a cargo de cada uno de los usuarios. El mantenimiento de las instalaciones situadas entre la llave de paso del edificio y los contadores corresponde al propietario del inmueble o en la Comunidad de Propietarios.</p>
<p class="p-impressio">El cuarto de contadores será accesible sólo para el portero o vigilante y el personal de la compañía suministradora o de mantenimiento. Hay que vigilar que las rejas de ventilación no estén obstruidas, así como también el acceso a la cámara.</p>
<p class="p-impressio">Precauciones:</p>
<p class="p-impressio">Se recomienda cerrar la llave de paso de la vivienda en caso de ausencia prolongada. Si la ausencia ha sido muy larga se revisarán las juntas antes de abrir la llave de paso.</p>
<p class="p-impressio">Todas las fugas o defectos de funcionamiento a las conducciones, los accesorios o los equipos se repararán inmediatamente.</p>
<p class="p-impressio">Todas las canalizaciones metálicas se conectarán a la red de puesta a tierra. Está prohibido utilizar las tuberías como elementos de contacto de las instalaciones eléctricas con el suelo.</p>
<p class="p-impressio">Para desatascar tuberías, no se deben utilizar objetos punzantes que puedan perforar la foto.</p>
<p class="p-impressio">En caso de bajas temperaturas, se debe dejar correr el agua por las tuberías para evitar que se hiele el agua en su interior.</p>
<p class="p-impressio">El correcto funcionamiento de la red de agua caliente es uno de los factores que influyen más decididamente en el ahorro de energía, de ahí que debe ser objeto de una mayor atención para obtener un rendimiento energético óptimo.</p>
<p class="p-impressio">En la revisión general se comprobará el estado del aislamiento y la señalización de la red de agua, la estanqueidad de las uniones y las juntas, y el correcto funcionamiento de las llaves de paso y válvulas, y hay verificar la posibilidad de cierre total o parcial de la red.</p>
<div class="salt-pagina"></div>'

  	when 'electricitat'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">La instalación eléctrica está formada por el contador, para la derivación individual, por el cuadro general de mando y protección y por los circuitos de distribución interior. Al mismo tiempo, el cuadro general de mando y protección está formado por un interruptor de control de potencia (ICP), un interruptor diferencial (ID) y los pequeños interruptores automáticos (PIA).</p>
<p class="p-impressio">El ICP es el mecanismo que controla la potencia que suministra la red de compañía. El ICP desconecta la instalación cuando la potencia consumida es superior a la contratada o bien cuando se produce un cortocircuito (contacto directo entre dos hilos conductores) y el PIA de su circuito no se dispara previamente.</p>
<p class="p-impressio">El interruptor diferencial (ID) protege contra las fugas accidentales de corriente como, por ejemplo, las que se producen cuando se toca con el dedo un enchufe o cuando un hilo eléctrico toca una tubería de agua o la carcasa de la lavadora. El interruptor diferencial (ID) es indispensable para evitar accidentes. Siempre que se produce un escape salta el interruptor.</p>
<p class="p-impressio">Cada circuito de distribución interior tiene asignado un PIA que salta cuando el consumo del circuito es superior al previsto. Este interruptor protege contra los cortocircuitos y las sobrecargas.</p>
<p class="p-impressio">Responsabilidades:</p>
<p class="p-impressio">El mantenimiento de la instalación eléctrica a partir del contador (y no sólo desde el cuadro general de entrada a la vivienda) es a cargo de cada uno de los usuarios.</p>
<p class="p-impressio">El mantenimiento de la instalación entre la caja general de protección y los contadores corresponde al propietario del inmueble o en la Comunidad de Propietarios. Aunque la instalación eléctrica tiene desgastes muy pequeños, difíciles de apreciar, es conveniente realizar revisiones periódicas para comprobar el buen funcionamiento de los mecanismos y el estado del cableado, de las conexiones y del aislamiento. En la revisión general de la instalación eléctrica se debe verificar la canalización de las derivaciones individuales comprobando el estado de los conductos, fijaciones, aislamiento y tapas de registro, y verificar la ausencia de humedad.</p>
<p class="p-impressio">El cuarto de contadores será accesible sólo para el portero o vigilante, y el personal de la compañía suministradora o de mantenimiento. Hay que vigilar que las rejas de ventilación no estén obstruidas, así como también el acceso a la cámara.</p>
<p class="p-impressio">Precauciones:
<p class="p-impressio">Las instalaciones eléctricas deben utilizarse con precaución por el peligro que conllevan. Está prohibido manipular los circuitos y los cuadros generales, estas operaciones deben ser efectuadas exclusivamente por personal especialista.</p>
<p class="p-impressio">No se debe permitir a los niños manipular los aparatos eléctricos cuando estén enchufados y, en general, se debe evitar manipularlos con las manos húmedas. Se debe tener especial cuidado en las instalaciones de baños y cocinas (locales húmedos).</p>
<p class="p-impressio">No se pueden conectar a los enchufes aparatos de potencia superior a la prevista o varios aparatos que, en conjunto, tengan una potencia superior. Si se aprecia un calentamiento de los cables o de los enchufes conectados a un determinado punto, se deben desconectar. Es síntoma de que la instalación está sobrecargada o no está preparada para recibir el aparato. Las clavijas de los enchufes deben estar bien atornilladas para evitar que hagan chispas. Las conexiones malas originan calentamientos que pueden generar un incendio.</p>
<p class="p-impressio">Es recomendable cerrar el interruptor de control de potencia (ICP) de la vivienda en caso de ausencia prolongada. Si se deja el frigorífico en funcionamiento, no es posible desconectar el interruptor de control de potencia, pero sí cerrar los pequeños interruptores automáticos de otros circuitos.</p>
<p class="p-impressio">Periódicamente, es recomendable pulsar el botón de prueba del diferencial (ID), el cual debe desconectar toda la instalación. Si no la desconecta, el cuadro no ofrece protección y habrá que avisar al instalador.</p>
<p class="p-impressio">Para limpiar las luces y las placas de los mecanismos eléctricos se debe desconectar la instalación eléctrica. Se deben limpiar con un paño ligeramente humedecido con agua y detergente. La electricidad se conectará una vez se hayan secado las placas.</p>
<div class="salt-pagina"></div>'

  	when 'climatitzacio'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Se deben leer y seguir las instrucciones de la instalación antes de ponerla en funcionamiento por primera vez.</p>
<p class="p-impressio">El correcto funcionamiento de la instalación es uno de los factores que influyen más decisivamente en el ahorro de energía, por lo tanto se debe mantener con cuidado para obtener un rendimiento óptimo.</p>
<p class="p-impressio">Hay que informar al personal de mantenimiento si se detectan fugas de agua en los aparatos emisores o en las conducciones y se procederá a su reparación inmediata. En su caso se avisará del hecho a la Propiedad.</p>
<p class="p-impressio">Los aparatos emisores de calor no deben taparse con ningún mueble u objeto.</p>
<p class="p-impressio">Después de una fuga de gas, no se puede encender la calefacción hasta que el instalador haya revisada.</p>
<div class="salt-pagina"></div>'

  	when 'gas'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">El grado de peligrosidad de las instalaciones de gas combustible es superior a cualquier otra. Por esta razón se extremarán las medidas de seguridad. Cualquier intervención sobre la instalación o sobre los aparatos de consumo debe estar hecha por una empresa especializada.</p>
<p class="p-impressio">Hay que seguir las instrucciones de mantenimiento de los aparatos de gas que proporcionan sus fabricantes.</p>
<p class="p-impressio">No se pueden observar las chimeneas de evacuación de humos, ni se pueden dejar objetos cerca de focos de calor o superficies calientes.</p>
<p class="p-impressio">No se pueden tapar los agujeros de ventilación de los espacios que contienen instalaciones de gas. Debe haber ventilación tanto en la parte alta como en la parte baja del espacio.</p>
<p class="p-impressio">Los gases propano y butano son más pesados que el aire y, por tanto, en caso de fuga van hacia abajo. El gas natural es menos pesado que el aire y, por tanto, en caso de fuga va hacia arriba.</p>
<p class="p-impressio">Si se quiere dar suministro a otros aparatos que los de construcción original se debe pedir permiso a la Propiedad. La instalación del nuevo aparato debe hacer una empresa autorizada.</p>
<p class="p-impressio">Los quemadores deben mantenerse limpios de depósitos y residuos de la combustión. Se comprobarán periódicamente las emisiones de gases contaminantes y si es necesario se harán los ajustes y reparaciones en los aparatos para que estas se mantengan dentro de los parámetros establecidos.</p>
<p class="p-impressio">Se recomienda que en ausencias largas se cierre la llave de paso general de la instalación de gas. Durante la noche es mejor hacer lo mismo, siempre que no quede en funcionamiento ningún aparato de gas.</p>
<p class="p-impressio">El mantenimiento de las instalaciones situadas entre el grifo de entrada del inmueble y el contador corresponde a la Propiedad.</p>
<p class="p-impressio">Los tubos de gas no debe usarse para las tomas de tierra de los aparatos eléctricos ni tampoco para colgar objetos.</p>
<p class="p-impressio">El cuarto de contadores sólo será accesible al portero, al vigilante, o al personal de la compañía suministradora y el de mantenimiento. Hay que vigilar que tanto las rejas de ventilación como el acceso a la cámara no queden obstruidos.</p>
<p class="p-impressio">Si se detecta una fuga de gas no debe abrirse ninguna luz ni accionar interruptores o conectar aparatos eléctricos. La conexión o desconexión de aparatos eléctricos puede provocar chispas que a su vez pueden provocar la deflagración del gas acumulado. Tampoco se pueden utilizar encendedores ni encender cerillas. En estos casos se intentará cerrar la llave de paso del gas y luego se ventilará el recinto donde se ha producido la fuga abriendo puertas y ventanas de par en par. Finalmente se debe avisar de la avería a una empresa instaladora autorizada o bien al servicio de urgencias de la empresa suministradora para que localice la avería.</p>
<div class="salt-pagina"></div>'

  	when 'ventilacio'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Se deben ventilar los espacios interiores para evitar humedades de condensación, especialmente si la calefacción es de estufas de gas butano. La ventilación se realizará preferentemente en horas de sol, y por espacio de 20 o 30 minutos.</p>
<p class="p-impressio">Los hongos que aparecen en los puntos más fríos de los recintos tienen su origen en humedades de condensación y no en goteras, como se cree normalmente. La limpieza debe realizarse con productos fungicidas, pero hay que tener en cuenta que de no resolver el problema de origen, los hongos volverán a aparecer si no se mejora la ventilación o el aislamiento térmico de la pieza.</p>
<p class="p-impressio">En caso de aumentar el aislamiento térmico de forma puntual sin reducir la humedad ambiental, la condensación se producirá en otro punto frío provocando deterioros de los revestimientos.</p>
<p class="p-impressio">No se permite conectar extractores a conductos de evacuación de gases de calderas de calefacción.</p>
<p class="p-impressio">Hay que leer y seguir las instrucciones de los aparatos de extracción mecánica.</p>
<div class="salt-pagina"></div>'

  	when 'incendis'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Las instrucciones de uso de los componentes de la instalación figuran en los mismos aparatos. Todos los componentes de la instalación deben estar en su sitio y al alcance. No se han de ocultar.</p>
<p class="p-impressio">Queda completamente prohibido hacer un uso indebido de los elementos de la instalación contra incendios.</p>
<p class="p-impressio">Se debe mantener la instalación de protección al fuego en condiciones reglamentarias. Todas las tareas de mantenimiento, revisión y reparación deben estar hechas por una empresa autorizada.</p>
<p class="p-impressio">En caso de incendio:</p>
<p class="p-impressio">Si se produce un conato de incendio, hay que avisar a los bomberos ya los ocupantes del inmueble inmediatamente. Durante los primeros minutos de inicio del fuego, es posible intentar controlarlo con los medios de que disponga el edificio (mangueras, extintores). En este caso siempre deben actuar dos personas con un extintor cada una o con una manguera atacando al fuego. Nunca se debe actuar solo. Si pasados unos instantes no se ha dominado el fuego, se aconseja evacuar el edificio siguiendo el plan previsto de evacuación del inmueble.</p>
<p class="p-impressio">Si en algún conato de incendio se ha producido una descarga parcial o total de un extintor, se han desarrollado mangueras o se han accionado pulsadores de disparadores de alarma, hay que hacerlo saber al conserje o el personal de mantenimiento del edificio para que lleven a cabo las revisiones pertinentes y restablezcan el estado normal de la instalación.</p>
<div class="salt-pagina"></div>'

  	when 'ascensor'
  		return '<p class="apartat-sistema">Instrucciones de uso:</p>
<p class="p-impressio">Los ascensores no pueden ser utilizados por niños que no vayan acompañados de personas adultas.</p>
<p class="p-impressio">Si se observa que falla algún mecanismo, se debe prestar el servicio y avisar al encargado del mantenimiento.</p>
<p class="p-impressio">Los ascensores no se pueden utilizar como montacargas.</p>
<p class="p-impressio">No se puede fumar dentro de la cabina del ascensor.</p>
<p class="p-impressio">El conserje, portero, presidente de la comunidad o a quien el presidente delegue, es responsable del seguimiento de estas recomendaciones de uso.</p>
<p class="p-impressio">En caso de avería, de falta de suministro eléctrico, etc. es obligatorio tener conectada la cabina con la empresa de mantenimiento.</p>
<p class="p-impressio">Personas atrapadas en el interior por una avería</p>
<p class="p-impressio">Si alguna persona queda atrapada en el interior de la cabina debido a una avería, es fundamental no perder la calma. Hay que pulsar el timbre de alarma de la botonera de cabina o comunicar el hecho por teléfono, si se dispone de uno. Nunca se debe intentar abrir las puertas manualmente para salir de la cabina.</p>
<p class="p-impressio">Si la avería es un corte de suministro eléctrico, la cabina se detendrá y se encenderá la luz de emergencia. Hay que recordar que en este supuesto el ascensor es completamente seguro. Si en caso de avería no funciona el timbre de alarma, hay que llamar periódicamente para que se sepa la presencia de una persona en la cabina. Ninguna persona atrapada en el interior de una cabina debe intentar salir forzando las puertas, y menos cuando el ascensor se haya parado entre dos plantas.</p>
<p class="p-impressio">Las personas que están fuera del ascensor no deben manipular ningún mecanismo ni ninguna puerta. Se limitarán a avisar a la empresa de mantenimiento o, en un caso grave, a los bomberos. Si es posible, conviene informar de lo que pasa a la persona atrapada vigilando que mantenga la calma.</p>
<div class="salt-pagina"></div>'
