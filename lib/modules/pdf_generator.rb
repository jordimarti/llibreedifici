module PdfGenerator
  ...
  def descripcio_pdf(sistema, sistema_usuari)
    ...
    if sistema_usuari == 'tancaments' && @edifici.tancaments_vertical.sobrecarrega_repartida_balco != nil
      llistat_elements = llistat_elements + '<li>Sobrecàrrega repartida del balcó: ' + @edifici.tancaments_vertical.sobrecarrega_repartida_balco.to_s + ' kg/m<sup>2</sup></li>'
    end
    if sistema_usuari == 'tancaments' && @edifici.tancaments_vertical.sobrecarrega_linial_vora_balco != nil
      llistat_elements = llistat_elements + '<li>Sobrecàrrega linial actuant en la vora de la balconada: ' + @edifici.tancaments_vertical.sobrecarrega_linial_vora_balco.to_s + ' kg/m<sup>2</sup></li>'
    end
    #Tanquem el llistat
...
<div class="salt-pagina"></div>'
  	...
  	when 'estructura'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Les humitats persistents als elements estructurals tenen un efecte nefast sobre la conservació de l’estructura. Cal reparar-les immediatament.</p>
<p>Si s’han de penjar objectes (quadres, prestatgeries, mobles o lluminàries) als elements estructurals cal utilitzar tacs i cargols adequats per al material de base.</p>
<p>Els elements que formen part de l’estructura de l’edifici, parets de càrrega incloses, no es poden alterar sense el control del Tècnic de Capçalera. Aquesta prescripció inclou la realització de regates a les parets de càrrega i l’obertura de passos per a la redistribució d’espais interiors.</p>
<p>Durant la vida útil de l’edifici poden aparèixer símptomes de lesions a l’estructura o als elements en contacte amb ella. En general, aquests defectes poden tenir caràcter greu. En aquests casos, és necessari que el vostre Tècnic de Capçalera analitzi les lesions detectades, en determini la importància i, si escau, decideixi la necessitat d’una intervenció.</p>
<p>Relació orientativa de símptomes de lesions amb repercussió possible sobre l’estructura:
<ul>
<li>Deformacions: desploms de parets, façanes i pilars.</li>
...
<div class="salt-pagina"></div>'
  	...
  	when 'tancaments'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Les façanes separen l’habitatge de l’ambient exterior, per aquest motiu, han de complir importants exigències d’aïllament respecte del fred o la calor, el soroll, l’entrada d’aire i d’humitat, la resistència, la seguretat al robatori, etc.</p>
<p>La façana constitueix la imatge externa de la casa i dels seus ocupants, conforma el carrer i, per tant, configura l’aspecte de la nostra ciutat. Per això, no pot alterar-se (tancar balcons amb vidre, obrir noves obertures, instal·lar tendals o rètols no apropiats) sense tenir en compte les ordenances municipals i l’aprovació de la Comunitat de Propietaris.</p> 
<p>Els acabats de la façana acostumen a ser uns dels punts més fràgils de l’edifici ja que estan en contacte directe amb la intempèrie. D’altra banda, el que inicialment potser només és brutícia o una degradació de la imatge estètica de la façana, es pot convertir en un perill, ja que qualsevol despreniment cauria directament sobre el carrer. </p>
<p>Els aplacats de pedra natural s’embruten amb molta facilitat depenent de la porositat de la pedra. Consulteu al vostre Tècnic de Capçalera la possibilitat d’aplicar un producte protector incolor. </p>
<p>Les parets mitgeres són aquelles que separen l’edifici dels edificis veïns. Quan no hi hagi edificis veïns o siguin més baixos, les mitgeres quedaran a la vista i hauran d’estar protegides com si fossin façanes. </p>
<p>Als balcons i les galeries no s’han de col·locar càrregues pesades, com ara jardineres o materials emmagatzemats. També s’hauria d’evitar que l’aigua que s’utilitza per regar regalimi per la façana. </p>
<p>Les finestres i balcons exteriors són elements comuns de l’edifici, tot i que el seu ús sigui majoritàriament privat. Qualsevol modificació de la seva imatge exterior (incloent-hi el canvi de perfileria) haurà de ser aprovada per la Comunitat de Propietaris. Tanmateix, la neteja i el manteniment correspon als usuaris dels habitatges.</p>
<p>No es recolzaran, sobre les finestres i balcons, elements de subjecció de bastides, politges per aixecar càrregues o mobles, mecanismes de neteja exteriors o d’altres objectes que els puguin malmetre.</p>
<p>No s’han de donar cops forts a les finestres. D’altra banda, les finestres poden aconseguir una alta estanquitat a l’aire i al soroll amb la col·locació de ribets especialment concebuts per aquesta finalitat.</p>
<p>Els vidres s’hauran de netejar amb aigua sabonosa, preferentment tèbia, i posteriorment s’assecaran. No s’han de fregar amb draps secs, ja que el vidre es ratllaria. </p>
<p>A les persianes enrotllables de PVC, s’ha d’evitar forçar les lamel·les quan perdin l’horitzontalitat o es quedin encallades a les guies. S’ha de netejar amb detergents no alcalins i aigua calenta. Cal utilitzar un drap suau o una esponja. </p>
<p>Una manca d’aïllament tèrmic pot ser la causa de l’existència d’humitats de condensació. El Tècnic de Capçalera haurà d’analitzar els símptomes adequadament per tal de determinar possibles defectes a l’aïllament tèrmic.</p>
<p>Si l’aïllament tèrmic es mulla, perd la seva efectivitat. Per tant, s’ha d’evitar qualsevol tipus d’humitat que el pugui afectar.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'cobertes'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Cobertes planes:</p>
<p>Les cobertes planes s’han de mantenir netes i sense herbes, especialment les buneres, les canals i els aiguafons. És preferible no col·locar jardineres a prop dels desguassos o bé que estiguin elevades del sòl per permetre el pas de l’aigua.</p>
<p>Aquest tipus de coberta només ha d’utilitzar-se per a l’ús a què hagi estat projectada. En aquest sentit, s’evitarà l’emmagatzematge de materials, mobles, etc., i l’abocament de productes químics agressius com ara olis, dissolvents o lleixiu.</p>
<p>Si a la coberta s’hi instal·len noves antenes, equips d’aire condicionat o, en general, aparells que requereixin ser fixats, la subjecció no ha d’afectar la impermeabilització.</p>
<p>Tampoc no s’han d’utilitzar com a punts d’ancoratge de tensors, de baranes metàl·liques o d’obra, ni de conductes d’evacuació de fums existents, llevat que el Tècnic de Capçalera ho autoritzi. Si aquestes noves instal·lacions precisen d’un manteniment periòdic, es preveuran al seu entorn les proteccions adequades. </p>
<p>En cas que s’observin humitats als pisos sota coberta, caldrà controlar-les, ja que poden tenir un efecte negatiu sobre els elements estructurals. </p>
<p>Cal procurar, sempre que sigui possible, no caminar per sobre de les cobertes planes no transitables. Quan sigui necessari trepitjar-les s’ha d’anar amb molt de compte per no produir desperfectes. El personal d’inspecció, conservació o reparació estarà proveït de sabates de sola tova. </p>
<p>Cobertes amb pendent:</p>
<p>Les cobertes s’han de mantenir netes i sense herbes, especialment les buneres, les canals i els aiguafons. S’ha de procurar, sempre que sigui possible, no trepitjar les cobertes en pendent. Quan s\'hi transiti cal anar amb molt de compte per no produir desperfectes.</p>
<p>Les cobertes en pendent seran accessibles només per a la seva conservació. El personal encarregat del treball anirà proveït de cinturó de seguretat que subjectarà a dos ganxos de servei o a punts fixos de la coberta. És recomanable que els operaris portin sabates amb soles toves i antilliscants. No es transitarà sobre les cobertes si estan mullades.</p>
...
<p>En el cas que s’observin humitats a les plantes sota coberta, s’hauran de controlar, ja que poden tenir un efecte negatiu sobre els elements estructurals.</p>
<p>La molsa i els fongs s’eliminaran amb un raspall i si cal s’aplicarà un fungicida.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'aigua'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Responsabilitats:</p>
<p>El manteniment de la instal·lació a partir del comptador (no només des de la clau de pas de l\'habitatge) és a càrrec de cadascun dels usuaris. El manteniment de les instal·lacions situades entre la clau de pas de l\'edifici i els comptadors correspon al propietari de l\'immoble o a la Comunitat de Propietaris.</p>
<p>La cambra de comptadors serà accessible només per al porter o vigilant i el personal de la companyia subministradora o de manteniment. Cal vigilar que les reixes de ventilació no estiguin obstruïdes, així com també l’accés a la cambra.</p>
<p>Precaucions:</p>
<p>Es recomana tancar la clau de pas de l\'habitatge en cas d\'absència prolongada. Si l\'absència ha estat molt llarga s\'han de revisar els junts abans d\'obrir la clau de pas.</p>
<p>Totes les fuites o defectes de funcionament a les conduccions, els accessoris o els equips es repararan immediatament.</p>
<p>Totes les canalitzacions metàl·liques es connectaran a la xarxa de posada a terra. És prohibit d’utilitzar les canonades com a elements de contacte de les instal·lacions elèctriques amb el terra.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'electricitat'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>La instal·lació elèctrica està formada pel comptador, per la derivació individual, pel quadre general de comandament i protecció i pels circuits de distribució interior. Al mateix temps, el quadre general de comandament i protecció està format per un interruptor de control de potència (ICP), un interruptor diferencial (ID) i els petits interruptors automàtics (PIA).</p>
<p>L’ICP és el mecanisme que controla la potència que subministra la xarxa de companyia. L’ICP desconnecta la instal·lació quan la potència consumida és superior a la contractada o bé quan es produeix un curtcircuit (contacte directe entre dos fils conductors) i el PIA del seu circuit no es dispara prèviament.</p>
<p>L’interruptor diferencial (ID) protegeix contra les fuites accidentals de corrent com, per exemple, les que es produeixen quan es toca amb el dit un endoll o quan un fil elèctric toca una canonada d’aigua o la carcassa de la rentadora. L’interruptor diferencial (ID) és indispensable per tal d’evitar accidents. Sempre que es produeix una fuita salta l’interruptor.</p>
<p>Cada circuit de distribució interior té assignat un PIA que salta quan el consum del circuit és superior al previst. Aquest interruptor protegeix contra els curtcircuits i les sobrecàrregues.</p>
<p>Responsabilitats:</p>
<p>El manteniment de la instal·lació elèctrica a partir del comptador (i no només des del quadre general d’entrada a l’habitatge) és a càrrec de cadascun dels usuaris.</p>
<p>El manteniment de la instal·lació entre la caixa general de protecció i els comptadors correspon al propietari de l’immoble o a la Comunitat de Propietaris. Tot i que la instal·lació elèctrica té desgasts molt petits, difícils d’apreciar, és convenient fer revisions periòdiques per tal de comprovar el bon funcionament dels mecanismes i l’estat del cablejat, de les connexions i de l’aïllament. A la revisió general de la instal·lació elèctrica s’ha de verificar la canalització de les derivacions individuals comprovant l’estat dels conductes, fixacions, aïllament i tapes de registre, i verificar l’absència d’humitat.</p>
<p>La cambra de comptadors serà accessible només per al porter o vigilant, i el personal de la companyia subministradora o de manteniment. S’ha de vigilar que les reixes de ventilació no estiguin obstruïdes, així com també l’accés a la cambra.</p>
<p>Precaucions:
<p>Les instal·lacions elèctriques s’han d’utilitzar amb precaució pel perill que comporten. Està prohibit manipular els circuits i els quadres generals, aquestes operacions han de ser efectuades exclusivament per personal especialista.</p>
<p>No s’ha de permetre als nens manipular els aparells elèctrics quan estiguin endollats i, en general, s’ha d’evitar manipular-los amb les mans humides. S’ha de tenir especial cura en les instal·lacions de banys i cuines (locals humits).</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'climatitzacio'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>S’han de llegir i seguir les instruccions de la instal·lació abans de posar-la en funcionament per primera vegada.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'gas'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>El grau de perillositat de les instal·lacions de gas combustible és superior a qualsevol altra. Per aquesta raó s’extremaran les mesures de seguretat. Qualsevol intervenció sobre la instal·lació o sobre els aparells de consum ha d’estar feta per una empresa especialitzada.</p>
<p>Cal seguir les instruccions de manteniment dels aparells de gas que proporcionen els seus fabricants.</p>
<p>No es poden obstruir les xemeneies d’evacuació de fums, ni es poden deixar objectes a prop de focus de calor o superfícies calentes.</p>
<p>No es poden tapar els forats de ventilació dels espais que contenen instal·lacions de gas. Hi ha d\'haver ventilació tant a la part alta com a la part baixa de l’espai.</p>
<p>Els gasos propà i butà són més pesants que l\'aire i, per tant, en cas de fuita van cap avall. El gas natural és menys pesant que l\'aire i, per tant, en cas de fuita va cap a amunt.</p>
<p>Si es vol donar subministrament a uns altres aparells que els de construcció original s\'ha de demanar permís a la Propietat. La instal·lació del nou aparell l\'ha de fer una empresa autoritzada.</p>
<p>Els cremadors s’han de mantenir nets de dipòsits i residus de la combustió. Es comprovaran periòdicament les emissions de gasos contaminants i si és necessari es faran els ajustos i les reparacions en els aparells per tal de que aquestes es mantinguin dintre dels paràmetres establerts.</p>
<p>Es recomana que en absències llargues es tanqui l\'aixeta de pas general de la instal·lació de gas. Durant la nit és millor fer el mateix, sempre que no quedi en funcionament cap aparell de gas.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'ventilacio'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>S’han de ventilar els espais interiors per evitar humitats de condensació, especialment si la calefacció és d\'estufes de gas butà. La ventilació s\'ha de fer preferentment en hores de sol, i per espai de 20 o 30 minuts.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'incendis'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Les instruccions d’ús dels components de la instal·lació figuren en els mateixos aparells. Tots els components de la instal·lació han d’estar al seu lloc i a l’abast. No s’han d’ocultar.</p>
<p>Queda completament prohibit fer un ús indegut dels elements de la instal·lació contra incendis.</p>
...
<div class="salt-pagina"></div>'
  	...
  	when 'ascensor'
  		return '<p class="apartat-sistema">Instruccions d’ús:</p>
<p>Els ascensors no poden ser utilitzats per nens que no vagin acompanyats de persones adultes.</p>
<p>Si s’observa que falla algun mecanisme, s’ha de parar el servei i avisar l’encarregat del manteniment.</p>
<p>Els ascensors no es poden utilitzar com a muntacàrregues.</p>
<p>No es pot fumar dins de la cabina de l’ascensor.</p>
<p>El conserge, porter, president de la comunitat o a qui el president delegui, és responsable del seguiment d’aquestes recomanacions d’ús.</p>
<p>En cas d’avaria, de manca de subministrament elèctric, etc. és obligatori tenir connectada la cabina amb l’empresa de manteniment.</p>
