class VentilacionsController < ApplicationController
  ...
  def edit
    ...
    if @edifici.ventilacio.id != params[:id].to_i
      redirect_to home_permisos_path
    end

    @submenu_actiu = 'sistemes'
    @menu_sistemes_actiu = 'ventilacio'
    @elements = Element.where(:edifici_id => @edifici.id, :sistema_element => 'ventilacio')
  end
